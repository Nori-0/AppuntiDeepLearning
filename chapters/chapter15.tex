...
Una delle implementazioni più celebri ed efficienti di questa famiglia è \textbf{Stable Diffusion}. La sua innovazione principale consiste nel non applicare la diffusione direttamente sui pixel, ma utilizzando uno \textbf{Spazio Latente Compresso}. Questo spazio, ottenuto tramite un Autoencoder, rappresenta le informazioni essenziali dell’immagine in una forma compatta e gestibile.
...
\begin{itemize}
    ...
    \item \textbf{\texttt{text+image}:} generazione o modifica di immagini a partire da un’immagine e un prompt testuale.
\end{itemize}
...
\begin{Osservazione}
    Per rendere il Training meno costoso, si possono utilizzare le stesse immagini con diverse quantità di rumore, così da poter avere un dataset più ampio, su cui allenare in Noise Predictor, a partire da poche immagini.
\end{Osservazione}
